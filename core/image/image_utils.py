...
def calculate_centroid_expansion_box(
    ...
    Args:
        ...
        target_center: Optional (x, y) target center point to use as the anchor
                       instead of the mask moment centroid. Typically the bubble
                       bounding box center. If provided and within the safe area,
                       it is used directly for ray-casting.
    ...
    try:
        ...
        if target_center is not None:
            centroid_x, centroid_y = target_center
            centroid_x = max(0.0, min(centroid_x, mask_w - 1))
            centroid_y = max(0.0, min(centroid_y, mask_h - 1))
        else:
